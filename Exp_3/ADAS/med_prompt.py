...
LLM_debate = {
    ...
    debate_agents = [LLMAgentBase(['thinking', 'answer'], 'Debate Agent', temperature=0.8, role=role) for role in ['Medical Doctor', 'Medical Student', 'Biomedical Researcher']]
...
Role_Assignment = {"thought": "Similar to Auto-GPT and expert prompting, we can use dynamic control flow in the design to let the agent decide what expert we should use.",
        ...
        # Instruction for step-by-step reasoning
        cot_instruction = "Please think step by step and then solve the task."
        expert_agents = [LLMAgentBase(['thinking', 'answer'], 'Expert Agent', role=role) for role in ['Medical Doctor', 'Biomedical Researcher', 'Medical Student', 'Helpful Assistant']]

        # Instruction for routing the task to the appropriate expert
        routing_instruction = "Given the task, please choose an Expert to answer the question. Choose from: Medical Doctor, Biomedical Researcher, Medical Student."
        routing_agent = LLMAgentBase(['choice'], 'Routing agent')

        # Get the choice of expert to route the task
        choice = routing_agent([taskInfo], routing_instruction)[0]

        if 'doctor' in choice.content.lower():
            expert_id = 0
        elif 'researcher' in choice.content.lower():
            expert_id = 1
        elif  'student' in choice.content.lower():
            expert_id = 2
        else:
...
base = """# Overview
You are an expert machine learning researcher testing various agentic systems. Your objective is to design building blocks such as prompts and control flows within these systems to solve complex tasks. Your aim is to design an optimal agent performing well on the Medical Abstract Classification Benchmark, which evaluates disease-related text understanding and category classification through challenging multiple-choice questions.

## An example question from Medical Abstract:

Determine the most likely disease category described in the abstract below:

**Abstract:**  
A 65-year-old man presents with progressive memory loss and disorientation. MRI reveals significant hippocampal atrophy. Neurological assessment shows difficulty with short-term recall, but no motor deficits. The patient has a family history of dementia.
...
The patient shows hallmark symptoms of neurodegeneration (memory loss, hippocampal atrophy), suggesting Alzheimer's or a related disorder. These conditions fall under the category of **nervous system diseases**. There are no signs of cancer (neoplasms), digestion-related problems, or cardiovascular issues.
...
class LLMAgentBase:
    ...
    def __init__(self, output_fields: list, agent_name: str, role='helpful assistant', model='gpt-4.1-nano', temperature=0.5) -> None:
