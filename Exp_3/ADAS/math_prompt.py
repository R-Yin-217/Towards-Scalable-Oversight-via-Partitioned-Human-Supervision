...
base = """# Overview
You are an expert machine learning researcher testing various agentic systems. Your objective is to design building blocks such as prompts and control flows within these systems to solve complex tasks. Your aim is to design an optimal agent performing well on the Math Benchmark, which evaluates symbolic, multi-step reasoning abilities through multiple-choice math problems.
...
But \\( x = 2 \\) is not allowed (denominator zero).  
...
So the equation is valid for all \\( x \\ne 2 \\), thus answer is:
